...
object ChassisKt{
    ...
    fun setMotion(x : Double, y : Double, rot : Double){
        ...
        val mxSpeed = maxOf(maxOf(abs(leftFront), abs(leftRear)), maxOf(abs(rightFront), abs(rightRear)))
        if (mxSpeed > 1.0){
            mecDrive.setMotorPowers(leftFront / mxSpeed, leftRear / mxSpeed, rightFront / mxSpeed, rightRear / mxSpeed)
        } else {
